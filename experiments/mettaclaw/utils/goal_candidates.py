...
GRAPH_FILENAME = "adagram_sparse_random.metta"


def _locate_default_graph() -> Path:
    """Walk up from this file to find experiments/data/, so the module keeps
    working wherever under experiments/ it is placed."""
    here = Path(__file__).resolve()
    for parent in here.parents:
        candidate = parent / "data" / GRAPH_FILENAME
        if candidate.is_file():
            return candidate
    return here.parent.parent / "data" / GRAPH_FILENAME


DEFAULT_GRAPH = _locate_default_graph()
GROUP_SIZE = 5
